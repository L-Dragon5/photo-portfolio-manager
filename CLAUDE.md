...
- **`search-docs`** — Search version-specific Laravel/ecosystem docs before making changes
- **`tinker`** — Execute PHP/Eloquent directly for debugging
- **`database-query`** / **`database-schema`** — Read DB data and inspect table structure
- **`browser-logs`** — Read browser console errors and exceptions
- **`list-artisan-commands`** — Verify Artisan command signatures before running
- **`get-absolute-url`** — Generate correct local URLs (served via Laravel Herd at `*.test`)
- **`last-error`** / **`read-log-entries`** — Inspect Laravel application errors
...
Inertia page components are in `resources/js/Pages/Public/` and `resources/js/Pages/Admin/`. Mantine 8 is the component library — use its components rather than HTML elements or custom CSS where possible.
